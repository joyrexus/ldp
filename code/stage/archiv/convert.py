...
class Transcript(datastore.table.Reader):
    """
    Use this class to convert incoming transcript files into canonical
    ...
    def __iter__(self):
        '''Yield prefixed row values.'''
        row = 0
        for v in self.values():
            row += 1
            id = self._pk(self.subj, self.sess, row)
            yield [id, self.subj, self.sess, row, ''] + list(v)


if __name__ == '__main__':

    header_printed = False                  # header not yet printed

    files = sys.argv[1:]
    for f in files:
        t = Transcript(f)
        if not header_printed:              # only print header once
            print "\t".join(t.header())
            header_printed = True
        for row in t: 
